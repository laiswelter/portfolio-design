...
// ── Shared asset-path resolver ────────────────────────────────────────────
// Single source of truth for every `assets/...` reference across the jsx
// components and case-study data files. In the standalone single-file
// bundle export, paths resolve via window.__resources (inlined blobs). On
// GitHub Pages (multi-file, served from /en/ or /pt/), it falls back to the
// real relative path, prefixed with window.ASSET_BASE so it still resolves
// correctly from a language subfolder (e.g. ASSET_BASE = '../').
window.bundledUrl = window.bundledUrl || function (path) {
  if (!path || typeof path !== 'string' || path.indexOf('assets/') !== 0) return path;
  var id = path.replace(/^assets\//, '').replace(/\.[^.]+$/, '');
  if (window.__resources && window.__resources[id]) return window.__resources[id];
  return (window.ASSET_BASE || '') + path;
};

(() => {
  const STATE_FILE = (window.ASSET_BASE || '') + '.image-slots.state.json';
  ...
  class ImageSlot extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._ro = new ResizeObserver(() => {
        ...
        this._roRaf = requestAnimationFrame(() => this._render());
      });
    ...
    _applyView() {
      ...
      if (fit !== 'cover' || !g) {
        ...
        // `hug`: size the host box to the image's own ratio so a `contain`
        // slot leaves no letterbox bands ("transparent border") around it.
        if (this.hasAttribute('hug') && fit === 'contain') {
          const iw = this._img.naturalWidth, ih = this._img.naturalHeight;
          if (iw && ih) this.style.aspectRatio = iw + ' / ' + ih;
        }
        return;
      }
